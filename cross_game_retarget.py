...
def _no_table_message(session_key, dest_arm):
    ...
    compose = _compose()
    if compose is None:
        # Without the composer, a same-named pair cannot be told apart -- and the
        # direct-bind path treats it as what it most likely is: the same skeleton
        # family, needing no table at all. A DIFFERENT-named pair genuinely cannot
        # be joined without the add-on, and that is the message worth sending.
