...
def _repair_clip_paths(clip, path_to_bone, avatar_json=None):
    import json
    aliases = clip_repair.avatar_path_aliases(json.loads(avatar_json), path_to_bone) if avatar_json else None
    return clip_repair.repair_hashed_clip_paths(clip, path_to_bone, aliases)
...
def _solve_humanoid_curves(clip, avatar_json, path_to_bone, warnings, clip_name):
    ...
    _repaired, unmatched = _repair_clip_paths(solved, path_to_bone, avatar_json)
    if unmatched:
...
def build_selected_animations(db, arm_obj, maps, path_to_meshobjects, guids, options,
    ...
    for guid in guids:
        ...
        repaired, unmatched = _repair_clip_paths(clip, path_to_bone, avatar_json)
        if unmatched:
...
def _apply_clip_paths(context, clip_paths, options):
    ...
    for clip_path in clip_paths:
        ...
        _repaired, unmatched = _repair_clip_paths(clip, path_to_bone, avatar_json)
        if unmatched:
