...
class RURI_PG_cabmap(filter_ui.FilterStateMixin, step_loader.LoadingState,
    ...
    # 0.45: the asset name is the one cell whose content is a long unique identifier
    # worth reading in full, and the reference's 240/960 truncated nearly every row --
    # but the container path is the second-longest cell and the old defaults starved it
    # to a twelfth of the row, so it is widened to about a quarter. Because the other
    # three factors are relative, these land on screen at roughly
    # Name .45 / Path .25 / Type .12 / Deps .05 / Source .13.
    col_name_factor: FloatProperty(
        name="Name", default=0.45, min=0.05, max=0.95, subtype="FACTOR",
        description="Width of the Name column")
    col_container_factor: FloatProperty(
        name="Path", default=0.45, min=0.05, max=0.95, subtype="FACTOR",
        description="Width of the Path (virtual container path) column")
    col_type_factor: FloatProperty(
        name="Type", default=0.4, min=0.05, max=0.95, subtype="FACTOR",
        description="Width of the Type column")
    col_deps_factor: FloatProperty(
        name="Deps", default=0.28, min=0.05, max=0.95, subtype="FACTOR",
        description="Width of the Deps column -- Source fills whatever's left")
    # How tall the file list is drawn. A browser is the one list worth giving the
    # screen to, and 12 rows of a 260k-row map was a keyhole.
    browser_rows: IntProperty(
        name="Rows", default=22, min=6, max=60,
        description="How many rows of the file list to show at once")
    ...
    import_action_chain: BoolProperty(
        name="Experimental Action Chain", default=False,
        description="Opt into the prototype Endfield NLA sequence; leave off for the selected raw clip only")
    ...
    def as_options(self, scene=False):
        ...
        return {
            ...
            "import_action_chain": self.import_action_chain,
...
class RURI_OT_import_selected(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if module is not None and module.importer is not None:
            return self._import_through_module(context, module, target_rows, state)
    ...
    def _import_through_module(self, context, module, target_rows, state):
        """Hand the selected packages to the module that knows how to read this build.

        A build the host does not read natively states its own importer (see Game.GameModule);
        the closure road below is what a Unity build needs and would be pure overhead here.
        """
        if self.reset_scene:
            bpy.ops.object.select_all(action="SELECT")
            bpy.ops.object.delete(use_global=False)
        packages = [row["cab"] for row in target_rows if row.get("cab")]
        try:
            built = module.importer(context, packages, state.as_options())
        except Exception as exc:
            _report_exception(self, "Import failed", exc)
            return {"CANCELLED"}
        if not built:
            self.report({"WARNING"}, "The {0} selected package(s) built nothing.".format(len(packages)))
            return {"CANCELLED"}
        self.report({"INFO"}, "{0} object(s) from {1} package(s).".format(len(built), len(packages)))
        return {"FINISHED"}
...
class RURI_PT_column_widths_popover(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.prop(state, "browser_rows")
    ...
    def draw(self, context):
        ...
        if active is None:
            ...
            gated.template_list(RURI_UL_cabmap.bl_idname, "", state, "window",
                                state, "active_index", rows=state.browser_rows)
...
def _maybe_build_endfield_action_chain(context, guids, db, arm_obj, maps, options, path_to_meshobjects=None):
    """Optional Endfield expansion; ordinary clips and other games are unchanged."""
    if not options.get("import_action_chain", False):
        return False, []
    try:
        if (cabmap_state.game_of(cabmap_state.active_key()) or "").lower() != "endfield":
            return False, []
        from .Game.EndField import action_chain
        return action_chain.maybe_build(
            context, guids, db, arm_obj, maps, options, path_to_meshobjects)
    except Exception as exc:
        return False, [f"Endfield action-chain build failed: {exc}"]
...
class RURI_OT_import_selected_animations(bpy.types.Operator):
    ...
    def execute(self, context):
            ...
            if target_maps is not None:
                ...
                if not checked or ratio > 0.0 or retargetable:
                    try:
                        ...
                        return {"CANCELLED"}
                    chain_built, chain_warnings = _maybe_build_endfield_action_chain(
                        context, selected_guids, clip_db, target, target_maps,
                        state.as_options(), None)
                    warnings.extend(chain_warnings)
                    for warning in warnings[:5]:
                        self.report({"WARNING"}, warning)
                    suffix = " and built its action chain" if chain_built else ""
                    self.report({"INFO"}, "Imported %d animation(s) onto '%s'%s."
                                          % (built, target.name, suffix))
                    return {"FINISHED"}
        ...
        try:
            ...
            return {"CANCELLED"}
        chain_built, chain_warnings = _maybe_build_endfield_action_chain(
            context, guids, build_state["db"], arm_obj, build_state["maps"],
            state.as_options(), build_state.get("path_to_meshobjects"))
        build_warnings.extend(chain_warnings)
        for warning in build_warnings[:5]:
            self.report({"WARNING"}, warning)
        suffix = " and built its action chain" if chain_built else ""
        self.report({"INFO"}, f"Built {built} animation action(s) on {arm_obj.name}{suffix}.")
        return {"FINISHED"}
...
class RURI_PT_animation_browser(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if (_active_game_name(state) or "").lower() == "endfield":
            layout.prop(state, "import_action_chain")
