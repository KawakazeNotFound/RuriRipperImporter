...
def build_action(clip, armature_obj, maps, path_to_meshobjects=None, options=None,
    ...
    # The reference importer keeps source sample indices as Blender frames.
    # This preserves the clip's native 60-Hz frame domain and matches the
    # controller/action timeline used by the Endfield import panel.
...
def adopt_action(owner, action, slot=None, scene=None, frame_range=True):
    ...
    if span is not None:
        ...
        if math.isfinite(rate) and rate > 0.0:
            # Curves use native sample indices. Playing a 60-Hz import in a
            # scene left at 24 fps stretches it to 2.5 times its authored time.
