...
def resolve_union(hierarchy_cabs, clip_cabs):
    """ONE resolve covering a mixed selection: hierarchy rows plus clip rows.

    A clip row's closure co-seeds most of what a character row's does, so
    resolving the two separately re-read everything the first had just read.
    Resolved together there is one statement for the selection, and what is a
    performance rather than a thing to place is the clip table's own answer --
    no root has to be attributed back to an archive to keep the two apart."""
    seeds = list(hierarchy_cabs)
    for cab in clip_cabs:
        if cab not in seeds:
            seeds.append(cab)
    database = StatementDatabase(seeds)
    return {"db": database, "roots": database.all_guids(),
            "seed_roots": {seed: database.token for seed in seeds},
            "clips_by_cab": {}, "scene_roots": []}
...
def part_correction(part):
    ...
    from ...Kernel.math3d import coordinate as unity_space
...
def avatar_skeleton(db, avatar_file):
    ...
    from ...Kernel.unity import avatar
...
class StatementDatabase:
    """What a resolved selection IS, as the reader states it.

    One selection is ONE statement however many roots it covers, so there is a
    single token here rather than a guid per asset: building it places every
    root the seeds state, which is what the loop that used to walk them was
    doing one at a time. The statement itself is what a builder reads."""

    __slots__ = ("seeds", "token", "_statement")

    def __init__(self, seeds):
        self.seeds = list(seeds)
        self.token = "|".join(self.seeds)
        self._statement = None

    @property
    def statement(self):
        if self._statement is None:
            from ...Host.Blender import packages as materialiser

            self._statement = materialiser.statement_for(None, self.seeds)
    ...
    def load_guid(self, guid):
        return self.token if guid == self.token else None

    def raw_text(self, _guid):
        return ""

    def texture_bytes(self, guid):
        for texture in self.statement.textures:
            if texture.key == guid or texture.name == guid:
                return texture.image
        return b""

    def clip_curves(self, _guid):
        return b""
