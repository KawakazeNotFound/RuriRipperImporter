...
def _quick_add(context, arguments):
    ...
    if not arguments["field"]:
        # Nothing to narrow BY. The menu always names a field; a call that does
        # not is a call rather than a click, and saying so beats leaving an empty
        # rule behind for the next redraw to fail on.
        return {"CANCELLED"}
    rule = state.filter_rules.add()
    rule.spec_key = spec.key
