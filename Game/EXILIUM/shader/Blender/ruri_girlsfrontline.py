...
本平台产物同批同 stamp(2e04fa5bbb28dee8):每个生成栈一个 <栈名>.blend(模板节点组库,codegen 期
...
class Stack:
    ...
    def _wire_capability(self, g, insts, cap, ctx):
        ...
        answer = builder(g, query, dict(ctx, results=cap['results']))
        if answer is None:
    ...
    # 顶点树的相机矩阵是树里现读的活动相机(生成器 GraphLowerer.HostCamera),相机动了几何自己跟着动;
    # 树读不到的只有输出像素数 —— 它是渲染设置不是相机。socket 名字只在这张表上点一次,建树与重灌同吃它。
    ...
    def _screen_size(scene):
        ...
        return {'screen_x': float(scene.render.resolution_x) * scale,
                'screen_y': float(scene.render.resolution_y) * scale}
    ...
    def _write_screen_sockets(self, node, screen):
        ...
        written = False
        for sock in node.inputs:
            if sock.name not in self.SCREEN_SOCKETS:
                continue
    ...
    def apply_vertex_stage(self, objects=None, camera=None):
        ...
        if scene.camera is None:
            print('[ruri-vertex] 场景无活动相机:描边壳照建,树里读不到相机时壳塌成退化面;设好 scene.camera 即生效。', flush=True)
