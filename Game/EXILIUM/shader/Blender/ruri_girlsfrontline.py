...
本平台产物同批同 stamp(37e70777cbcb1928):每个生成栈一个 <栈名>.blend(模板节点组库,codegen 期
headless 物化,导入期默认 append 进当前文件),外加**全平台唯一的**本文件 ruri_girlsfrontline.py ——
...
VIEW_SOCKETS = ('ruri_view_transform', 'ruri_view_projection', 'ruri_view_near', 'ruri_view_far', 'ruri_view_ortho')
...
class Stack:
    ...
    def _cull_mode(self, props, meta):
        ...
        if self.CULL_TWO_SIDED:
            ...
            stated = props.floats.get(switch)
            if stated is None:
    ...
    def provider(self, builder, props):
        ...
        mat['ruri_uber_shader_passes'] = [str(light_mode or name)
                                          for name, light_mode in getattr(props, 'shader_passes', ())]
    ...
    def vtx_writeback(self, depsgraph):
        # 这条挂在 depsgraph_update_post 上,每一拍都必须便宜:视点跟着视口走,几何每拍都在重算,而拧
    ...
    def _viewpoint_answers(self, tree, nd):
        """外层顶点树里答视点的节点,返回 {组输入名: 输出口};模板的同名组输入(VIEW_SOCKETS 与
        SCREEN_SOCKETS)接这里。视点 = 这一次求值为谁而算:视口里是宿主的视点物体(用户正看着的那个视图),
        渲染里是活动相机与渲染输出 —— 几何树只有 Is Viewport 分得清两者,没有读视口的节点。视点物体与它
        身上点属性的名字是宿主的契约(host.viewpoint()),这里一个都不点名。"""
        view = self._host_module().viewpoint()
        is_viewport = nd('GeometryNodeIsViewport').outputs[0]
        placed = nd('GeometryNodeObjectInfo')
        placed.transform_space = 'RELATIVE'
        placed.inputs['Object'].default_value = view.object
        carried = nd('GeometryNodeObjectInfo')
        carried.transform_space = 'ORIGINAL'
        carried.inputs['Object'].default_value = view.object

        def carried_value(name, data_type):
            attribute = nd('GeometryNodeInputNamedAttribute')
            attribute.data_type = data_type
            attribute.inputs['Name'].default_value = name
            node = nd('GeometryNodeSampleIndex')
            node.data_type = data_type
            node.domain = 'POINT'
            tree.links.new(carried.outputs['Geometry'], node.inputs['Geometry'])
            tree.links.new(attribute.outputs['Attribute'], node.inputs['Value'])
            return node.outputs[0]

        def split(vector):
            node = nd('ShaderNodeSeparateXYZ')
            tree.links.new(vector, node.inputs[0])
            return node.outputs

        camera = nd('GeometryNodeInputActiveCamera').outputs[0]
        camera_placed = nd('GeometryNodeObjectInfo')
        camera_placed.transform_space = 'RELATIVE'
        tree.links.new(camera, camera_placed.inputs['Object'])
        camera_info = nd('GeometryNodeCameraInfo')
        tree.links.new(camera, camera_info.inputs['Camera'])
        frame = split(carried_value(view.frame, 'FLOAT_VECTOR'))
        screen = split(carried_value(view.screen, 'FLOAT_VECTOR'))
        render_screen = split(carried_value(view.render_screen, 'FLOAT_VECTOR'))

        def pick(input_type, rendered, viewed):
            node = nd('GeometryNodeSwitch')
            node.input_type = input_type
            tree.links.new(is_viewport, node.inputs['Switch'])
            tree.links.new(rendered, node.inputs['False'])
            tree.links.new(viewed, node.inputs['True'])
            return node.outputs[0]

        return {
            VIEW_SOCKETS[0]: pick('MATRIX', camera_placed.outputs['Transform'], placed.outputs['Transform']),
            VIEW_SOCKETS[1]: pick('MATRIX', camera_info.outputs['Projection Matrix'],
                                  carried_value(view.projection, 'FLOAT4X4')),
            VIEW_SOCKETS[2]: pick('FLOAT', camera_info.outputs['Clip Start'], frame[0]),
            VIEW_SOCKETS[3]: pick('FLOAT', camera_info.outputs['Clip End'], frame[1]),
            VIEW_SOCKETS[4]: pick('FLOAT', camera_info.outputs['Is Orthographic'], frame[2]),
            SCREEN_SOCKETS[0]: pick('FLOAT', render_screen[0], screen[0]),
            SCREEN_SOCKETS[1]: pick('FLOAT', render_screen[1], screen[1]),
        }
    ...
    def apply_vertex_stage(self, objects=None, camera=None):
        ...
        视点(视口里用户正看着的那个视图、渲染里的活动相机)在树里现读,骨改名了只重接基座
        ...
        for obj in [o for o in (objects if objects is not None else scene.objects)
            ...
            # 它身上的壳与描边建出来也没人看得见,只是白付每一次视点变化的重算。
            ...
            def viewpoint_answers():
                if not viewpoint_cache:
                    viewpoint_cache.update(self._viewpoint_answers(mt, nd))
                return viewpoint_cache

            def wire_vertex_inputs(node):
                for sock in node.inputs:
                    if sock.name in VIEW_SOCKETS or sock.name in SCREEN_SOCKETS:
                        src = viewpoint_answers()[sock.name]
                    else:
            ...
            if outline_slots:
                ...
                for slot, _mat in outline_slots:
                    ...
                    if keep is None:
                        ...
                        keep = mx.outputs[0]
                # 描边是相对视点定义的:还没有视点(没有界面、场景也没有相机)时没有描边可言,壳整个不出。
                has_view = nd('ShaderNodeMath')
                has_view.operation = 'GREATER_THAN'
                mt.links.new(viewpoint_answers()[VIEW_SOCKETS[3]], has_view.inputs[0])
                has_view.inputs[1].default_value = 0.0
                viewed = nd('ShaderNodeMath')
                viewed.operation = 'MULTIPLY'
                mt.links.new(keep, viewed.inputs[0])
                mt.links.new(has_view.outputs[0], viewed.inputs[1])
    ...
    def _panel_bound_image(mat, slot):
        """材质自己的图节点只建在材质树顶层(_teximage;循环体也在同一棵树里),模板组里只有占位图 ——
        不下钻组:下钻是逐个没绑图的槽把整栈模板扫一遍,全场面板回读因此要一分多钟。"""
    ...
    def set_images(self, scene, images):
        """按 extra_images() 往本级组的图像口接图。images[口名] = (宽, 高, 逐纹素 RGBA 浮点平铺)。
        图是这个场景自己的数据(逐场景一张,Non-Color 浮点,打包进文件),接在场景合成树里的 Image 节点上;
        模板组不烙任何图。缺一口就拒绝,理由同 set_extra。"""
        ...
        if node is None:
            raise RuntimeError('[ruri-post] 场景的合成树里没有本级 {0}:先装本级再接图'.format(self.post['group']))
        tree = scene.compositing_node_group
        for name in names:
            ...
            if image is not None and tuple(image.size) != (width, height):
                ...
                image = None
            if image is None:
                image = bpy.data.images.new(image_name, width, height, alpha=True, float_buffer=True, is_data=True)
            image.colorspace_settings.name = 'Non-Color'
            image.pixels.foreach_set(texels)
            # 改了像素不 update,合成器读到的还是旧缓冲(实测全 0);存成 EXR 打包,重开文件仍是浮点。
            image.update()
    ...
    def _unbuilt_inputs(self, tree):
        """关着的图链没建:只长在它各趟上的输入口此刻无处可写(写下的值记在树上,链打开重建时照原样写回)。"""
        remembered = tree.get(self.POST_VALUES_KEY) or {}
        passes = {entry['group']: entry for entry in (self.post.get('passes') or {}).values()}
        names = set()
        for chain in self.post.get('chains') or []:
            if not self._chain_enabled(chain, remembered):
    ...
    def set_extra(self, scene, values):
        ...
        场景树上,图链按新尺寸或新分辨率档重建时照原样写回。缺一口就拒绝 —— 半份参数比不接更坏
        ...
        if self.stage_node(scene) is None:
        ...
        tree[self.POST_VALUES_KEY] = remembered
        if not self.refresh_chains(scene):
    ...
    def _chain_signature(self, scene, tree):
        """图链的拓扑由出图尺寸、各链的分辨率档与开关定;没写过分辨率档时按半分辨率(真源 BloomResolution.Half = 2),
        没写过开关时按关。"""
        remembered = tree.get(self.POST_VALUES_KEY) or {}
        ...
        return signature + [1.0 if self._chain_enabled(chain, remembered) else 0.0
                            for chain in (self.post.get('chains') or []) if chain.get('enable_input')]

    @staticmethod
    def _chain_enabled(chain, remembered):
        """链的开关:没有开关口的链恒开;有开关口的按关卡写下的值(没写过 = 关)。"""
        name = chain.get('enable_input')
        if not name:
            return True
        return name in remembered and float(remembered[name][0]) >= 0.5
    ...
    def _level_domain(self, tree, scene, label, level, size):
        ...
        image = bpy.data.images.new('{0} {1} level {2} {3}'.format(self.post['group'], label, level, scene.name),
    ...
    def _build_chains(self, scene, tree, render, stage):
        ...
        scene_color = None
        for chain in chains:
            if chain['kind'] == 'screen_space_reflection' and self._chain_enabled(chain, remembered):
                scene_color = self._screen_space_reflection(tree, scene, render, chain, width, height)
    ...
    def _screen_space_reflection(self, tree, scene, render, chain, width, height):
        ...
        camera = self._ssr_camera(tree, render, label)
        overlays = {}
    ...
    def _ssr_camera(self, tree, render, label):
        """反射链的相机量,在场景树里现答(与顶点腿的视点同一套):渲染时是活动相机(Camera Info 的投影与物体变换),
        ...
        transform = pick('MATRIX', placed.outputs['Transform'], viewed.outputs['Transform'])
        host_projection = split(pick('MATRIX', info.outputs['Projection Matrix'], carried.outputs['Matrix']))
    ...
    def refresh_chains(self, scene):
        """出图尺寸或分辨率档变了就重建图链(级数与各级尺寸都随它们);主入口不动,记下的值照原样写回各组。"""
        tree = bpy.data.node_groups.get(self.post['scene_tree'])
        if tree is None or scene.compositing_node_group is not tree or not self.post.get('chains'):
            return False
        if list(tree.get(self.POST_SIZE_KEY) or []) == self._chain_signature(scene, tree):
            return False
        ...
        remembered = tree.get(self.POST_VALUES_KEY)
        if remembered is not None:
            self._write_values(tree, {name: list(value) for name, value in remembered.items()})
        return True
    ...
    def register_host(self):
        ...
        # 顶点腿拆成两段落在两个事实上:建树只认导入,骨名只重接基座;视点在树里现读,不再有逐树重灌。
