...
本平台产物同批同 stamp(d4eb861ba8527d7d):每个生成栈一个 <栈名>.blend(模板节点组库,codegen 期
headless 物化),外加**全平台唯一的**本文件 ruri_girlsfrontline.py —— 运行时 + 全部栈清单
...
class Stack:
    ...
    def compile_all(self):
        ...
        try:
            for mat in pending:
                try:
                    replaced[mat] = self.recompile(mat)
                except Exception as exc:
    ...
    def sync_post_chains(scene, depsgraph):
        """场景合成树被改过 → 各后处理级重判图链,并把面板上拧过的旋钮记回场景(合成树是插件数据、存盘不带,
        旋钮只有记在场景上才随文件走)。面板拧的是主入口上的口,Blender 不给回调,只能依赖图落定后看
