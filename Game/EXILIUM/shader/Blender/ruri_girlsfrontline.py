...
本平台产物同批同 stamp(917a8ae924cb09e1):每个生成栈一个 <栈名>.blend(模板节点组库,codegen 期
...
class Stack:
    def __init__(self, folder, manifest):
        ...
        self.volume_textures = manifest.get('volume_textures') or {}
    ...
    def build_material(self, mat, part=None, opaque=True, multiply_blend=False, cull=2.0, images=None,
        ...
        surface = self._wire_closure(g, mat, color_sock, self._final_socket(insts, finals, NO_LOOP_FINAL))
    ...
    def _wire_closure(self, g, mat, color_sock, noloop_sock=None):
    ...
    def volume_texture_layouts(self):
        """本栈以 2D 图集兑现的 3D 纹理及其铺法(宿主据此建图、铺片、填边)。"""
        return dict(self.volume_textures)

    def _volume_image(self, slot):
        """3D 纹理图集:全场共享一张图(引擎全局,不随材质变)。图只有宿主一个建造者 —— 它也是写像素的那个,
        两处各建一份,尺寸/格式/色彩空间迟早分叉。"""
        return getattr(self._host_module(), self.host['volume_image_fn'])(self.volume_textures[slot])
    ...
    def _template_images(self, part):
        ...
        for fetch in rows:
            if fetch['slot'] in out:
                continue
            if fetch['slot'] in self.volume_textures:
                out[fetch['slot']] = self._volume_image(fetch['slot'])
                continue
    ...
    def rewire_capabilities(self, mat):
        ...
        if not self.PART_META.get(part, {}).get('multiply'):
            ...
            g._set(mix.inputs[2], self._wire_closure(
    ...
    def register_host(self):
        ...
        getattr(host, self.host['register_volume_textures_fn'])(self.volume_texture_layouts)
    ...
    def unregister_host(self):
        ...
        getattr(host, self.host['unregister_volume_textures_fn'])(self.volume_texture_layouts)
