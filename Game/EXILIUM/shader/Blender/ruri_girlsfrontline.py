...
本平台产物同批同 stamp(7626b212ab8dadeb):每个生成栈一个 <栈名>.blend(模板节点组库,codegen 期
...
class Stack:
    ...
    def _write_values(self, tree, values):
        ...
        chain_only = self._chain_inputs()
        sizes = self.post['sizes']
        for name, value in values.items():
            if not any([self._write_input(node, name, value, sizes[name]) for node in nodes]) and name not in chain_only:
    ...
    def _chain_signature(self, scene, tree):
        """图链的拓扑由出图尺寸与各链的分辨率档定;没写过分辨率档时按半分辨率(真源 BloomResolution.Half = 2)。"""
        remembered = tree.get(self.POST_VALUES_KEY) or {}
        width, height = self._output_size(scene)
        return [width, height] + [float(remembered[name][0]) if name in remembered else 2.0
    ...
    def _level_domain(self, tree, scene, chain, level, size):
        ...
        image = bpy.data.images.new('{0} {1} level {2} {3}'.format(self.post['group'], chain['image'], level, scene.name),
        ...
        domain[self.POST_CHAIN_KEY] = chain['image']
        coordinates[self.POST_CHAIN_KEY] = chain['image']
    ...
    def _build_chains(self, scene, tree, render, stage):
        ...
        for chain in self.post.get('chains') or []:
            ...
            tree.links.new(self._bloom_pyramid(tree, scene, render.outputs['Image'], chain, width, height, resolution),
    ...
    def _bloom_pyramid(self, tree, scene, source, chain, width, height, resolution):
        ...
        fields = [self._level_domain(tree, scene, chain, level, size) for level, size in enumerate(sizes)]
