...
本平台产物同批同 stamp(cd38f80e3f36a8c8):每个生成栈一个 <栈名>.blend(模板节点组库,codegen 期
...
def _cap_shadow_attenuation(g, query, ctx):
    ...
    询问带位置就在那个位置求值(真源把采样点沿光向推过一段时,推过的位置就是询问):单建一个节点,
    Position 口接询问位置 —— 共享的那个还要答不带位置的询问(Position 不接 = 着色点本身;
    真源的 shadowCoord 是管线私产,不参与询问)。出口按主光身份门进账,所以答的就是主光那一圈的阴影。"""
    _ = ctx
    ...
    if position is None:
        ...
        g._set(raycast.inputs['Position'], g.u2b(position))
    ...
    def set_extra(self, scene, values):
        ...
        for name, value in zip(names, values):
            ...
            if socket.type == 'VECTOR':
                # 二维形参由 z 恒 0 的向量口承载(生成器的载体约定),给两个分量就补上那个 0。
                value = tuple(value) + (0.0,) * (3 - len(value))
