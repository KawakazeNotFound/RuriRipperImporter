...
本平台产物同批同 stamp(8e5064c29f08bd24):每个生成栈一个 <栈名>.blend(模板节点组库,codegen 期
...
class Stack:
    ...
    def _unbuilt_inputs(self, tree):
        ...
        for chain in self.post.get('chains') or []:
            if not self._chain_enabled(tree, chain):
    ...
    def _chain_signature(self, scene, tree):
        ...
        return signature + [1.0 if self._chain_enabled(tree, chain) else 0.0
                            for chain in (self.post.get('chains') or []) if chain.get('enable_input')]

    def _chain_enabled(self, tree, chain):
        ...
        if not name:
            return True
        for node in self._stage_nodes(tree):
            if node.node_tree.name != self.post['group']:
                continue
            socket = node.inputs.get(name)
    ...
    def _build_chains(self, scene, tree, render, stage):
        ...
        scene_color = None
        for chain in chains:
            if chain['kind'] == 'screen_space_reflection' and self._chain_enabled(tree, chain):
        ...
        for chain in chains:
            if chain['kind'] != 'bloom_pyramid' or not self._chain_enabled(tree, chain):
                continue
    ...
    def refresh_chains(self, scene):
        ...
        if stage is None or stage.node_tree.get('ruri_stamp') != self.STAMP:
            # 上一代产物装的树:主入口是上一代的组,拿本代的趟往上接是两代混建,而本代一取组上一代整批让名,主入口
            # 从此按名找不到。原样留着(它自己仍自洽),响亮喊一次;后处理面板上重装本级才用上本代的链。
            if not self._stale_said.get(tree.name):
                self._stale_said[tree.name] = True
                print('[ruri-post] 合成树 {0} 是旧产物装的(stamp {1} ≠ {2}),不重建图链;后处理面板上重装本级后生效'.format(
                    tree.name, None if stage is None else stage.node_tree.get('ruri_stamp'), self.STAMP), flush=True)
            return False
    ...
    def sync_post_chains(scene, depsgraph):
        """场景合成树被改过 → 各后处理级重判图链。面板拧的是主入口上的口,Blender 不给回调,只能依赖图落定后看
        这一拍更新的数据块里有没有这棵树;没被改就一行都不多付(转视角不产生依赖图更新)。"""
        tree = scene.compositing_node_group
        if tree is None or not any(getattr(update.id, 'original', None) == tree for update in depsgraph.updates):
            return
        for stack in STACKS:
