...
本平台产物同批同 stamp(6a28e9664dab279a):每个生成栈一个 <栈名>.blend(模板节点组库,codegen 期
...
def _cap_opaque_distance_behind(g, query, ctx):
    ...
    法线按宿主的口径翻回:宿主在背面把插值法线整个取反(eevee_surf_common 的 init_globals),而这一点立着的
    那张面不随毛片被看到的是哪一面翻转。相机前向在宿主的相机空间里是 +Z:Vector Transform 从相机空间出发时
    先把 Z 取反(node_shader_vector_transform 的 invert_z,照 Cycles 的相机系)。"""
    normal = query.get('surface_normal')
    if normal is None:
        return None
    geo = g.geo()
    incoming = geo.outputs['Incoming']
    side = g.math('SUBTRACT', 1.0, g.math('MULTIPLY', geo.outputs['Backfacing'], 2.0))
    rise = g.vmath('DOT_PRODUCT', incoming, g.vmath('SCALE', g.u2b(normal), s=side))
...
def _prepare_surface_height(obj, asking, ground):
    ...
    询问它的几何是立在面上的卡:每张 asking 面上离 ground 那些面最近的顶点是这张面的根,过根、以顶点法线为法向的平面
    就是它立着的那张面 —— 高度 = (顶点 − 根) · 顶点法线 + 根离面的距离;一个顶点落在几张面上时取根最贴面的那一张。
    ...
    if grounded.any() and standing.any():
        ...
        for vertex in _np.unique(_np.concatenate([corner_vertex[start:start + total]
            ...
            hit = tree.find_nearest(Vector(world[vertex]))
            if hit[0] is not None:
                clearance[vertex] = hit[3]
        anchor = {}
        for start, total in spans:
            ...
            if not known:
                continue
            root = min(known, key=clearance.get)
