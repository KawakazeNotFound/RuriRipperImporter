...
本平台产物同批同 stamp(d6b06b83f844a2d2):每个生成栈一个 <栈名>.blend(模板节点组库,codegen 期
headless 物化),外加**全平台唯一的**本文件 ruri_azurpromilia.py —— 运行时 + 全部栈清单
...
# 灯物体上宿主替这盏灯维护的那一格,着色器经 Attribute(LIGHT) 逐灯读回:xyz = 灯的世界旋转四元数的虚部(实部取非负,
# 按单位长还原),w = 是不是主光。原生光循环没有下标、没有名字,也读不到灯的朝向 —— 这两样是原生灯节点给不出、
# 只能由灯自己带着的;朝向随灯的变换、主光随场上的灯集合由 refresh_main_light_role 重写,挪灯/换色零重接。
LIGHT_STATE_ATTRIBUTE = 'ruri_light_state'
# 主光朝向那一格,场景上的视层属性(宿主经 refresh_main_light_role 写):方向光 = (朝灯方向, 0),点光/聚光 = (位置, 1)。
# 主光着色只读它的方向 —— 它与光循环这一圈是哪盏灯无关,宿主因此把整块主光着色放在循环外只算一次;颜色与阴影
# 仍是循环这一圈的原生量,出口的主光身份门保证只有主光那一圈进账。
MAIN_LIGHT_VECTOR_ATTRIBUTE = 'ruri_main_light_vector'
...
def refresh_main_light_role():
    """把宿主替每盏灯维护的那一格写到灯物体上(LIGHT_STATE_ATTRIBUTE):灯的世界朝向与主光身份;主光朝向那一格
    写到场景上(MAIN_LIGHT_VECTOR_ATTRIBUTE)。返回主光。只写原始数据块:ID 属性一变,依赖图重新求值,EEVEE 下一次
    同步就把属性带进去;值没变不写,免得在 depsgraph handler 里自激。场上没有灯时先立兜底 Sun,原生光循环才有圈可转。"""
    _ensure_fallback_light()
    main = _choose_main_light()
    scene = bpy.context.scene
    if main is None:
        vector = [FALLBACK_LIGHT_DIRECTION[0], FALLBACK_LIGHT_DIRECTION[1], FALLBACK_LIGHT_DIRECTION[2], 0.0]
    elif main.data.type == 'SUN':
        # EEVEE 的方向光朝灯方向 = 物体世界矩阵归一后的 +Z 轴(Light::sync 的 z_axis)。
        axis = main.matrix_world.to_3x3().normalized().col[2]
        vector = [axis.x, axis.y, axis.z, 0.0]
    else:
        position = main.matrix_world.translation
        vector = [position.x, position.y, position.z, 1.0]
    current = scene.get(MAIN_LIGHT_VECTOR_ATTRIBUTE)
    if current is None or [float(value) for value in current] != vector:
        scene[MAIN_LIGHT_VECTOR_ATTRIBUTE] = vector
    for obj in scene.objects:
        if obj.type != 'LIGHT':
            continue
        rotation = obj.matrix_world.to_3x3().normalized().to_quaternion()
        if rotation.w < 0.0:
            rotation.negate()
        state = [rotation.x, rotation.y, rotation.z, 1.0 if obj is main else 0.0]
        current = obj.get(LIGHT_STATE_ATTRIBUTE)
        if current is None or [float(value) for value in current] != state:
            obj[LIGHT_STATE_ATTRIBUTE] = state
    return main
...
def _native_light(g):
    ...
    零驱动器、零灯表、零探针。这里交与位置无关的那几样:灯心与颜色(Light Info)、宿主替它维护的那一格
    (LIGHT_STATE_ATTRIBUTE:朝向与主光身份)。颜色乘进源管线灯表按镜头距离给的那一份淡出(_camera_fade)。
    唯一不从这一圈取的是主光的方向(_main_light_direction):主光着色读它,读这一圈就得每盏灯把整块主光着色算一遍。
    ...
    state = _find_native_node(g, 'ShaderNodeAttribute', LIGHT_STATE_ATTRIBUTE)
    ...
    answer = {
        ...
        'is_main': state.outputs['Alpha'],
        'rotation': state.outputs['Vector'],
    }
...
def _light_axes(g):
    ...
    朝向是宿主维护的那一格:四元数虚部 (x, y, z),实部 w = sqrt(1 - |xyz|²)。"""
    hit = g._cse.get(('light_axes',))
    if hit is not None:
        return hit
    rotation = _native_light(g)['rotation']
...
def _main_light_direction(g):
    """主光朝灯的方向(Blender 世界系):场景上那一格(MAIN_LIGHT_VECTOR_ATTRIBUTE),方向光直接是方向,点光/聚光按位置
    与着色点求 —— 与 Light Evaluation 对主光那一圈给的方向同值。它不读光循环这一圈的灯,整块主光着色因此在循环外
    只算一次,而不是每盏灯算一遍再在出口抵掉。"""
    hit = g._cse.get(('main_light_direction',))
    if hit is not None:
        return hit
    vector = g.layer_attr(MAIN_LIGHT_VECTOR_ATTRIBUTE)
    toward = g.vmath('SUBTRACT', vector.outputs['Vector'],
                     g.vmath('SCALE', g.geo().outputs['Position'], s=vector.outputs['Alpha']))
    direction = g.vmath('NORMALIZE', toward)
    g._cse[('main_light_direction',)] = direction
    return direction


def _cap_main_light(g, query, ctx):
    """主方向光兑现(EEVEE):方向是场景上主光那一格(_main_light_direction),颜色是当前这一圈的原生灯。哪一圈算主光
    由出口的身份门裁决(Diffuse Light × 灯上的主光标记):别的圈拿别的灯的颜色算出的主光那一份在出口与「循环没跑」
    的那份相减抵掉,只有主光那一圈进账 —— 所以方向可以不随圈变,颜色与阴影(ShadowAttenuation)仍是这一圈的原生量。
    距离衰减与阴影衰减恒 1 是这条能力自己的语义(URP 的 GetMainLight:距离衰减是 unity_LightData.z,
    没被剔除就是 1;无参形不带阴影),不是降级。"""
    _ = (query, ctx)
    return {
        'direction': g.b2u(_main_light_direction(g)),
...
def _cap_punctual_light(g, query, ctx):
    """第 index 盏点光/聚光本身(EEVEE):宿主光循环这一圈的灯,index 恒 0(盏数恒 1,见 _cap_additional_light_count)。
    灯心、颜色、范围(截止距离)、锥斜坡与是不是聚光全是原生量,三根轴是宿主维护的朝向;源内核拿它们按自己的
    衰减窗口、锥角平方、管光与 cookie 去算。方向光那一圈颜色乘零 —— 它不是点光,贡献走主光链。"""
    _ = (query, ctx)
...
def _cap_source_light_parameters(g, query, ctx):
    """第 index 盏灯带着的源管线逐灯参数(EEVEE):宿主光循环这一圈的灯上、宿主照关卡陈述写下的那几个向量属性,
...
CAP_BUILDERS = {
    ...
    'PunctualLight': {EEVEE_ENGINE: _cap_punctual_light},
    'SourceLightParameters': {EEVEE_ENGINE: _cap_source_light_parameters},
...
def _linked_group(blend_path, stamp, wanted, name, adopt):
    ...
    (合成树找主入口)撞上别的就是两代混建。这一次带进来的数据块交给 adopt 认领一次。"""
...
class Stack:
    ...
    def _require_budget(self, mat, matrix=None):
        ...
        tree = mat.node_tree
        key = (part, len(tree.nodes), len(tree.links), getattr(bpy.context.scene.render, 'engine', ''), switches)
    ...
    def _wire_closure(self, g, mat, insts, finals, alpha_sock=None, coverage_sock=None):
        """表面闭包是兑现面的一部分:它随引擎走(EEVEE = Emission + Light Accumulation,其余 = Emission),
        ...
        surface = self._surface_closure(
            g, engine, weighted(color_sock, lit_scale),
    ...
    def _per_light_share(self, g, lit, unlit, noloop):
        ...
        是逐灯的;不读灯的那一份(环境、自发光……)已在循环之外经 Emission 进过账(见 _surface_closure)。

        主光那一圈交「终色 − 无灯终色」= 主光着色(附加光项在那一圈被身份门乘零,不会与主光重复);其余每一圈交
        「终色 − 同一条终色但附加光循环没跑过」—— 正好是这盏灯自己的附加光贡献,同圈里那些与它无关的项
        (按这盏灯算出来的主光着色、环境、自发光)在相减时逐位抵消,不必知道内核怎么把那一项合进去。

        没有附加光循环的 part 发不出后一份终色,只在主光那一圈进账(点光对它本来就没贡献)。"""
    ...
    def _surface_closure(self, g, engine, lit, noloop, unlit, glossy):
        """表面闭包(各份都已乘好混合与覆盖率)。EEVEE:不读灯的那一份 —— 无灯终色,终色不读灯时就是终色本身 —— 经
        ...
        只在主光那一圈进账;打开漫反射 / 光泽光照通道的视图层按瓣分开读,不读灯的那一份落在自发光通道。
        两个闭包实测都不钳值,HDR 原样通过。其余引擎:Emission(两瓣相加)。"""
        if engine != EEVEE_ENGINE:
            emission = g._nd('ShaderNodeEmission')
            total = lit if glossy is None else (glossy if lit is None else g.vmath('ADD', lit, glossy))
            if total is not None:
                g._set(emission.inputs[0], total)
    ...
    SUPERSEDED_KEYS = ('ruri_uber_stamp', 'ruri_param_col', 'ruri_uber_signature')
    ...
    def recompile(self, mat):
        ...
        compiled = set(self.RECORD_KEYS) | set(self.SUPERSEDED_KEYS) | {
            'ruri_uber_stack', self.BLEND_KEY, self.CLOSURE_ENGINE_KEY, self.SHADING_KEY}
        for key in mat.keys():
    ...
    def compile_all(self):
        ...
        if linked:
            print('[ruri-uber] !! {0} 张本栈材质是 link 进来的(只读):它们引用的模板组是插件数据,库文件里没存,'
    ...
    def apply_vertex_stage(self, objects=None, camera=None, refill=False):
        ...
        **只由「刚从游戏导入的这批对象」触发**:这里建的是拓扑,而拓扑的判据(这张材质有没有描边 pass、
        宽度是不是 0)说的是导入那一刻的游戏真值。
        ...
        for obj in [o for o in (objects if objects is not None else scene.objects)
            ...
            # ② 反壳描边 = 同树虚拟几何:本体原样复制一份、删无描边面、写 ruri_outline 面属性、Join 回本体。
