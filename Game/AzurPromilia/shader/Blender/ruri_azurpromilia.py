...
本平台产物同批同 stamp(da4ab4a9a5f8e320):每个生成栈一个 <栈名>.blend(模板节点组库,codegen 期
...
def refresh_main_light_role():
    """把宿主替每盏灯维护的那一格写到灯物体上(LIGHT_STATE_ATTRIBUTE):灯的世界朝向与主光身份。返回主光。
    只写原始数据块:ID 属性一变,依赖图重新求值该灯,EEVEE 下一次同步就把属性带进灯的属性表;值没变不写,
    免得在 depsgraph handler 里自激。场上没有灯时先立兜底 Sun,原生光循环才有圈可转。"""
