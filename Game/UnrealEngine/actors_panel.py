...
from ... import cabmap_panel, unreal_importer
...
class RURI_OT_unreal_actor_import(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        try:
            objects = unreal_importer.import_package(
                context, cabmap_state.BRIDGE, entry.package, options)
        except Exception as exc:
            self.report({"ERROR"}, "Unreal actor import: {0}: {1}".format(type(exc).__name__, exc))
            import traceback
            traceback.print_exc()
            return {"CANCELLED"}
        if not objects:
            self.report({"ERROR"}, "'{0}' places nothing this install carries.".format(entry.package))
            return {"CANCELLED"}
        self.report({"INFO"}, "{0} object(s) from '{1}'.".format(len(objects), entry.name))
        return {"FINISHED"}
