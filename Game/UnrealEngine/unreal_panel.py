...
Browsing the install's levels is NOT here: a level is content, not session state, so it
lives on this module's Scene tab (``scene_panel``) next to the Actors tab, in the shape
the engine itself ships levels in.
"""
