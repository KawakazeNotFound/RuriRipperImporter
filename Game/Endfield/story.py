...
def _load_unit(context, arguments):
    ...
    # yield from, not a hand-rolled loop: the loop forwards the steps but throws
    # away what the host sends back and what the builder RETURNS, so the caller
    # ends up holding the last read's result instead of the stage.
    stage = yield from stage_module.build_steps(context, state.unit, language=_language(),
