...
# charId -> {"model", "tag", "asset"}, read once per session from the hook's
# container-path model projection. Module scope for the same reason the tables are.
_CHARACTER_MODELS = {}


def _character_model(character_id):
    """The model prefab route the hook indexed for a character, or "" when its
    model is absent from the loaded cabmap.

    The hook publishes this once from the CAB map. Blender reads that direct
    projection rather than exporting each character's MonoBehaviour assets.
    """
    if not _CHARACTER_MODELS:
        _CHARACTER_MODELS.update(datasets.character_models())
...
def _as_character(member, lod):
    ...
    if not character:
        return None
    # ``part_rows`` intentionally returns every model family for a shared
    # character id. A character load, however, must import exactly the family
    # requested by the roster (postmodel by default), not both the in-world and
    # UI prefabs in one closure.
    family = member.get("family") or "postmodel"
    rows = datasets.model_rows(character, family, cast=CHARACTERS)
    if not rows and family != "postmodel":
...
def _as_template(member, lod):
    """An NPC: use its authored assembly when present, otherwise its direct prefab."""
    template = member.get("template") or ""
    if not template:
        return None
    manifest, hits, missing = model_parts(template, lod)
    if manifest is not None and hits:
    ...
    # 1.5.3 also ships complete NPC prefabs; map them directly when the config
    # assembly manifest is unavailable.
    rows = datasets.model_rows(template, "postmodel", cast=NPCS)
    if not rows:
        rows = datasets.model_rows(template, "uimodel", cast=NPCS)
    if not rows:
        return None
    chosen, _level = _at_detail_level(rows, lod)
    return Loadable(template, member.get("label") or template, PREFAB,
                    [row["cab"] for row in chosen])
...
def _build_prefab(context, loadable, resolved, options):
    ...
    The Endfield exporter does not preserve the source AssetBundle container path
    on its generated prefab path, so seed-root lookup is intentionally relaxed at
    this boundary.  The resolved closure here comes from this member's own prefab
    CABs; importing its exported roots is the reliable identity link."""
    from ... import cabmap_panel
    warnings = []
    # A cast member must remain animatable even when the generic asset browser's
    # saved mesh-only option is disabled in an older .blend. Do not mutate that
    # browser preference: override only this character-load operation.
    options = dict(context.scene.ruri_cabmap.as_options() if options is None else options)
    ...
    _ok, imported = cabmap_panel.import_hierarchy_from_closure(
        ...
        only_seeded=False, options=options)
