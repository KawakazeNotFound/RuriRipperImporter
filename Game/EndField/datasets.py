...
def character_model_cabs():
    """The CABs holding playable-character prefabs, retained for host diagnostics."""
    return _column(MODEL_ASSETS, "cab")


def character_models():
    """{character id: {model, tag, asset}} from the backend's direct projection.

    The backend discovers model identity while it scans AssetBundle containers, so
    resolving a roster entry no longer exports or parses MonoBehaviour YAML in Blender.
    """
    return {row["characterId"]: {"model": row["model"], "tag": row["tag"], "asset": row["asset"]}
            for row in _rows(CHARACTER_MODELS)}


def _animation_name(name):
    """Turn a roster/model token into the character id used by the animation table.

    The model tab keeps the imported family suffix (``_postmodel`` or
    ``_uimodel``), while the game's animation library is keyed by the shared
    character id.  Accepting either token keeps the two tabs composable.
    """
    value = str(name or "").replace("\\", "/").rstrip("/").rsplit("/", 1)[-1]
    for suffix in ("_postmodel", "_uimodel"):
        if value.lower().endswith(suffix):
            return value[:-len(suffix)]
    return value


def _animation_body_token(name):
    """The folder token used by Endfield's body-animation tree."""
    value = _animation_name(name)
    if value.lower().startswith("chr_"):
        parts = value.split("_")
        return "_".join(parts[2:]) if len(parts) >= 3 else ""
    separator = value.rfind("_")
...
def animation_anchor(name, cast):
    ...
    # Prefer the loaded map's own paths. This keeps Find Animations independent
    # of whether the active hook build publishes the optional projection table.
    local = _animation_anchor_from_cabmap(name)
    if local:
        return local
    try:
        rows = _rows(ANIMATIONS, name=_animation_name(name), cast=cast)
    except Exception:
        # Keep the button usable with an older hook that predates the published
        # animation projection. The already-loaded cabmap still contains the
        # authoritative container paths, so this is a local projection fallback.
        rows = []
    if not rows:
        return _animation_anchor_from_cabmap(name)
    row = rows[0]
    return {"anchor": row["anchor"], "hits": _int(row["hits"]), "group": row["group"]}


def _animation_anchor_from_cabmap(name):
    """Recover one body-animation anchor from loaded cabmap rows.

    This covers cabmaps opened with a hook build that predates
    ``endfield.character.animations``. It only inspects AnimationClip rows and
    returns the same shape as the published dataset.
    """
    token = _animation_body_token(name).lower()
    if not token or not len(cabmap_state.ROWS):
        return None
    cache_key = (id(cabmap_state.ROWS), token)
    if cache_key in _ANIMATION_FALLBACK_CACHE:
        return _ANIMATION_FALLBACK_CACHE[cache_key]
    marker = "/{0}/animations/".format(token)
    paths = []
    for row in cabmap_state.ROWS:
        ...
        if at < 0 or "/entity/actor/" not in lowered:
            continue
        paths.append(path)
    if not paths:
        return None
    first = sorted(set(paths), key=str.lower)[0]
    at = first.lower().find(marker)
    anchor = first[:at + len(marker)].rstrip("/")
    pieces = anchor.replace("\\", "/").split("/")
    try:
        actor = next(index for index, value in enumerate(pieces)
                     if value.lower() == "actor")
        group = pieces[actor + 1] if actor + 1 < len(pieces) else ""
    except StopIteration:
        group = ""
    result = {"anchor": anchor, "hits": len(set(paths)), "group": group}
    _ANIMATION_FALLBACK_CACHE[cache_key] = result
    return result
