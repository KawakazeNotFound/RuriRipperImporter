...
def resolve_rig(context):
    ...
    if rig is not None:
        return rig, ""
    # A loaded Endfield model can leave more than one armature in the scene
    # while Blender's active-object context is temporarily owned by the
    # Properties/Outliner area.  The model token is the game's own identity,
    # so use it to select the unique matching rig before asking the user.
    token = (getattr(context.scene.ruri_character, "character_token", "") or "").lower()
    if token:
        armatures = [obj for obj in context.scene.objects if obj.type == "ARMATURE"]
        matches = [obj for obj in armatures
                   if token in obj.name.lower() or obj.name.lower() in token]
        if len(matches) == 1:
            return matches[0], ""
