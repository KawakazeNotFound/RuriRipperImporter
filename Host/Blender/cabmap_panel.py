...
from ...Kernel.unity import (bridge_asset_db, class_registry, clip_paths,
...
def _import_single_asset(op, context, state, db, guid, class_name, name):
    ...
    if class_name == "Material":
        # One material on its own is the same statement every other row makes,
        # asked for one key: the stack that claims it decides what it becomes.
        from . import material_builder, packages as materialiser

        statement = db.statement if db is not None else materialiser.statement_for(
            context, [guid], state.as_options())
        builder = material_builder.MaterialBuilder(statement, state.as_options())
        made = [builder.build(key) for key in statement.materials.keys()]
        made = [one for one in made if one is not None]
        _sync_texture_roles(state)
        if not made:
            op.report({"ERROR"}, "Material failed to build -- see console.")
            return {"CANCELLED"}
        op.report({"INFO"}, "Imported material '{0}' (browse it in the material "
                            "list).".format(made[0].name))
        return {"FINISHED"}
...
class RURI_OT_import_selected_animations(bpy.types.Operator):
    ...
    def execute(self, context):
        """Build the checked performances onto the skeleton in front of the user.

        A clip arrives from the reader already re-anchored onto that skeleton's
        own bone paths, with a muscle encoding solved against the avatar the rig
        carries and a foreign skeleton's names mapped through -- so there is one
        path here for every title and no scoring, closure walking or asset
        database on this side. Where there is no skeleton yet, the same checked
        rows state the character too: it is placed first, then played onto."""
        state = context.scene.ruri_cabmap
        ...
        # This engine ships no AnimationClip in the first place -- an AnimSequence
        # never was one -- so it reads its own performances off the decoder.
        config = _active_config(state)
        ...
        options = state.as_options()
        target = prefab_importer.find_target_armature(context)
        if target is None:
            build_state = cabmap_state.ANIMATION_BUILD_STATE or {}
            seeds = list(build_state.get("seed_cabs") or []) or list(checked_keys)
            placed = materialiser.materialise(
                context, loading.Packages(seeds[0] if seeds else "", "", loading.PREFAB, seeds),
                None, options)
            for warning in placed.warnings[:5]:
                self.report({"WARNING"}, warning)
            target = placed.armature or prefab_importer.find_target_armature(context)
            if target is None:
                self.report({"ERROR"}, "The checked row(s) state no skeleton to play onto, and "
                                       "nothing in the scene is selected to play onto instead.")
                return {"CANCELLED"}

        built = materialiser.build_clips(context, "", checked_keys, None, options,
                                         activate=True)
        if not built:
            self.report({"ERROR"}, "The checked row(s) state no performance -- see console.")
            return {"CANCELLED"}
        self.report({"INFO"}, "Imported {0} animation(s) onto '{1}'.".format(built, target.name))
        return {"FINISHED"}
