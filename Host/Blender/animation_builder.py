...
def build_action(clip, armature_obj, maps, path_to_meshobjects=None, options=None,
    ...
    rot = _channels_by_path(clip.of_kind("rot"), "rotation", name)
    pos = _channels_by_path(clip.of_kind("pos"), "position", name)
    scale = _channels_by_path(clip.of_kind("scale"), "scale", name)
    euler = _channels_by_path(clip.of_kind("euler"), "euler", name)
...
def _apply_float_curves(action, clip, path_to_meshobjects, sample_rate, times):
    n = len(times)
    for channel in clip.of_kind("float"):
...
# ---------------------------------------------------------------------------
# What the host's Timeline protocol answers with
# ---------------------------------------------------------------------------
def play(context, rig, clips, options, activate=False):
    """Build every stated clip onto ``rig`` and, when asked, put the first one on
    it. Returns (built, lines).

    A clip arrives already re-anchored onto this rig's own bone paths with any
    muscle encoding solved against the avatar the rig carries, so there is one
    kind of curve to bake here and no solver in this host at all."""
    if not clips:
