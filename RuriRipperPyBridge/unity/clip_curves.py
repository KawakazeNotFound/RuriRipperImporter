...
class ClipCurves:
    ...
    __slots__ = ("name", "sample_rate", "start_time", "stop_time",
                 ...
                 "rotations", "positions", "scales", "eulers", "floats", "rest_position_paths")
    ...
    def from_blob(cls, meta_json, payload_bytes):
        ...
        clip.rest_position_paths = set(meta.get("restPositionPaths") or [])
...
def merge_solved(clip, solved, consumed_attributes):
    ...
    for existing, incoming in ((clip.rotations, solved.rotations),
        ...
        if existing is clip.positions:
            # Model defaults repair normalized imported rest offsets only when
            # the source has no authored position channel on that path.
            authored = {channel.path for channel in existing}
            incoming = [c for c in incoming
                        if c.path not in solved.rest_position_paths or c.path not in authored]
